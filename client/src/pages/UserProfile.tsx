  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 mt-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => status?.enabled ? disable2FA.mutate() : enable2FA.mutate()}
      ...
      {status?.enabled && (
        <div className="mt-4 p-3 bg-green-50 rounded-lg flex items-center gap-2">
          ...
          <span className="text-sm text-green-700">Active — a code will be sent to your email on each login</span>
        </div>
      )}
    </div>
  );
