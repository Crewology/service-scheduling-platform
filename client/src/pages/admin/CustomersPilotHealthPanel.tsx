...
export function CustomersPilotHealthPanel() {
  ...
  if (health.isLoading) {
    return <Card><CardContent className="p-8 text-sm text-muted-foreground">Checking Customers rollout health…</CardContent></Card>;
  }

  if (!health.data || health.error) {
    return <Card className="border-red-200"><CardContent className="p-8"><p className="font-semibold text-red-800">Customers rollout health could not be loaded</p><p className="mt-1 text-sm text-muted-foreground">{health.error?.message || "Owner access or database availability could not be confirmed."}</p><Button variant="outline" size="sm" className="mt-4" onClick={() => health.refetch()}><RefreshCw className="mr-2 h-4 w-4" />Try again</Button></CardContent></Card>;
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <CardTitle className="flex items-center gap-2"><ShieldCheck className="h-5 w-5" />Customers provider rollout</CardTitle>
                ...
                <Badge variant="outline" className="border-blue-200 bg-blue-50 text-blue-800">{data.audienceMode === "lifecycle_entitled" ? "Eligible providers" : "Private pilot"}</Badge>
              </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard icon={UsersRound} label="Customers providers" value={`${data.totals.activeProviders}/${data.totals.providers}`} detail="active / eligible audience" />
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader><CardTitle>Rollout flags</CardTitle><CardDescription>Monitoring only. No flag changes are available from this screen.</CardDescription></CardHeader>
      ...
      <Card>
        <CardHeader><CardTitle>Providers with Customers access</CardTitle><CardDescription>Provider-level aggregate metadata only; customer identities and private content are excluded.</CardDescription></CardHeader>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-[#174a73]"><SearchCheck className="h-5 w-5" /></span><div><CardTitle>Provider access review</CardTitle><CardDescription className="mt-1">Run a non-writing assessment of current lifecycle access and source eligibility. This screen cannot change provider access or rollout settings.</CardDescription></div></div>
        </CardHeader>
        <CardContent className="space-y-5">
          <form className="grid gap-4 rounded-2xl border bg-muted/20 p-4 lg:grid-cols-[minmax(180px,0.6fr)_minmax(260px,1fr)_auto] lg:items-end" onSubmit={event => { event.preventDefault(); const providerId = Number(candidateProviderId); if (!Number.isInteger(providerId) || providerId <= 0) return; if (candidateRequest?.providerId === providerId && candidateRequest.hasReachableTester === hasReachableTester) void candidate.refetch(); else setCandidateRequest({ providerId, hasReachableTester }); }}>
            <div><Label htmlFor="customers-candidate-provider">Provider ID</Label><Input id="customers-candidate-provider" className="mt-2 bg-white" inputMode="numeric" min={1} type="number" value={candidateProviderId} onChange={event => setCandidateProviderId(event.target.value)} placeholder="Enter provider ID" /></div>
              <div className="flex items-start justify-between gap-4 rounded-xl border bg-white p-3"><div><Label htmlFor="customers-candidate-tester">Reachable customer-side tester confirmed</Label><p className="mt-1 text-xs leading-5 text-muted-foreground">Optional owner-provided evidence only. The assessment does not contact anyone.</p></div><Switch id="customers-candidate-tester" checked={hasReachableTester} onCheckedChange={setHasReachableTester} /></div>
            <Button type="submit" disabled={!Number.isInteger(Number(candidateProviderId)) || Number(candidateProviderId) <= 0 || candidate.isFetching}>{candidate.isFetching && <RefreshCw className="mr-2 h-4 w-4 animate-spin" />}Review access</Button>
          </form>

          {candidate.error ? <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-900">Provider access could not be assessed. {candidate.error.message}</div> : null}
          {candidate.data ? <CandidateReadinessResult data={candidate.data} /> : <div className="rounded-xl border border-dashed p-5 text-sm leading-6 text-muted-foreground">Enter one provider ID to review current lifecycle access and aggregate projection evidence. No customer identities, private content, configuration changes, or access actions are returned.</div>}
        </CardContent>
      </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle>Out-of-scope safeguards</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-3 sm:grid-cols-4"><SmallMetric label="Enabled rules" value={data.future.enabledAutomationRules} /><SmallMetric label="Automation runs" value={data.future.automationRuns} /><SmallMetric label="Automation failures" value={data.future.failedAutomationRuns} /><SmallMetric label="Saved segments" value={data.future.savedSegments} /><p className="col-span-2 rounded-xl border border-blue-100 bg-blue-50 p-3 text-xs leading-5 text-blue-900 sm:col-span-4">No AI, recommendations, scheduled work, bulk messaging, exports, external CRM, or manual contacts are enabled by this rollout.</p></CardContent></Card>
      </div>
