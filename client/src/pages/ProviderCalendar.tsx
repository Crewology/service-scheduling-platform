...
export default function ProviderCalendar() {
      ...
      <div className="border rounded-lg overflow-hidden">
        <div className="grid grid-cols-[60px_repeat(7,1fr)] bg-muted/50 border-b">
          ...
          {weekDates.map((date) => {
            ...
            const isToday = dateKey === todayKey;
            return (
              <div key={dateKey} className={`text-center py-2 border-l ${isToday ? "bg-primary/10" : ""}`}>
          ...
          {hours.map((hour) => (
            <div key={hour} className="grid grid-cols-[60px_repeat(7,1fr)] min-h-[60px]">
              ...
              {weekDates.map((date) => {
                ...
                return (
                  <div key={`${dateKey}-${hour}`} className="border-l border-b relative min-h-[60px]">
  ...
  const stats = useMemo(() => {
    if (!calendarData) return { total: 0, pending: 0, confirmed: 0, completed: 0 };
