...
export async function generateSocialPost(): Promise<{ content: string; postType: string; categoryId?: number; categoryName?: string }> {
  ...
  switch (postType) {
    ...
    case "customer_attraction":
      prompt = `Write a compelling social media post for OlogyCrew (a service scheduling platform at ologycrew.com) to attract customers looking for "${randomCategory.name}" services. Be 2-3 sentences max, highlight factual benefits (provider evidence signals where available, easy booking, transparent pricing, reviews tied to completed OlogyCrew bookings), include a CTA to book at ologycrew.com, use 2-3 hashtags. Never call all providers verified or imply guarantees. Professional but engaging. Max 1-2 emojis. Return ONLY the post text.`;
      break;
