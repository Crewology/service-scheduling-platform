...
export default function ProviderCustomerDetail() {
  ...
  const [sendingDraft, setSendingDraft] = useState<CustomerMessageDraft | null>(null);
  ...
  const sendDraft = trpc.customers.sendDraft.useMutation({
    onSuccess: async result => {
      setSendingDraft(null);
      await refresh();
      toast.success(result.alreadySent ? "Message was already sent" : "In-app message sent");
    },
    onError: error => {
      setSendingDraft(null);
      refresh();
    ...
    <div className="container max-w-7xl py-5 pb-28 sm:py-8 lg:pb-10">
      ...
      <div className="grid gap-6 lg:grid-cols-[232px_minmax(0,1fr)]">
        ...
        <main className="min-w-0">
          ...
          {access.data.readOnly ? <section className="mt-5 flex flex-col gap-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-amber-950 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-start gap-3"><LockKeyhole className="mt-0.5 h-5 w-5 shrink-0" /><div><h2 className="font-semibold">Private tools are paused on your current plan</h2><p className="mt-1 text-sm leading-6 text-amber-900">You can still review this customer’s OlogyCrew history. Pro or Business is required to view or add private notes, manage follow-ups, use message drafts, or change the manual stage. Existing private records are retained and return when qualifying access is restored.</p></div></div><Button asChild variant="outline" className="shrink-0 border-amber-300 bg-white text-amber-950 hover:bg-amber-100"><Link href="/provider/subscription">Review plans</Link></Button></section> : null}
          ...
          {access.data.draftsEnabled && <section className="mt-6 rounded-3xl border border-violet-100 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-violet-700">Provider-reviewed</p><div className="mt-1 flex flex-wrap items-center gap-2"><h2 className="flex items-center gap-2 text-xl font-bold text-slate-950"><MessageSquareText className="h-5 w-5" />Message drafts</h2><Badge variant="outline" className={draftSendReadiness.enabled ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}>{draftSendReadiness.enabled ? "Confirmed sending" : "Sending disabled"}</Badge></div><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-600">Write and review a private draft first. Sending requires a separate confirmation and the customer’s current relationship-message permission.</p></div><Button onClick={openCreateDraft}><Plus className="mr-2 h-4 w-4" />New draft</Button></div>{draftSendReadiness.enabled && <div className={`mt-4 rounded-2xl border px-4 py-3 text-sm leading-6 ${draftSendReadiness.allowed ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-amber-200 bg-amber-50 text-amber-900"}`}>{draftSendReadiness.allowed ? "This customer currently allows provider relationship messages. Permission is checked again when you confirm Send." : draftSendReadiness.reason === "relationship_unavailable" ? "Sending is unavailable while this relationship is archived." : "This customer has not enabled relationship messages. You can keep drafts private, but you cannot send one."}</div>}<div className="mt-5 space-y-3">{drafts.length ? drafts.map(draft => <article key={draft.id} className="rounded-2xl border border-violet-100 bg-violet-50/40 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Badge variant="outline" className={draft.state === "sent" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-violet-200 bg-white text-violet-800"}>{draft.state === "sent" ? "Sent" : "Not sent"}</Badge><span className="text-xs text-slate-500">{draft.state === "sent" && draft.sentAt ? `Sent ${formatDate(draft.sentAt)}` : `Updated ${formatDate(draft.updatedAt)}`}</span></div><p className="mt-3 whitespace-pre-wrap break-words text-sm leading-6 text-slate-800">{draft.body}</p></div><div className="flex shrink-0 flex-wrap gap-2">{draft.state === "sent" ? draft.conversationHref && <Button asChild variant="outline" size="sm"><Link href={draft.conversationHref}><ArrowRight className="mr-1.5 h-3.5 w-3.5" />View conversation</Link></Button> : <><Button variant="outline" size="sm" onClick={() => openEditDraft(draft)}><Pencil className="mr-1.5 h-3.5 w-3.5" />Edit</Button><Button variant="outline" size="sm" className="border-red-200 text-red-700 hover:bg-red-50 hover:text-red-800" onClick={() => setDiscardingDraft(draft)}><Trash2 className="mr-1.5 h-3.5 w-3.5" />Discard</Button>{draftSendReadiness.enabled && <Button size="sm" onClick={() => setSendingDraft(draft)} disabled={!draftSendReadiness.allowed}><Send className="mr-1.5 h-3.5 w-3.5" />Review & send</Button>}</>}</div></div></article>) : <EmptyTool icon={MessageSquareText} title="No message drafts yet" text="Create an unsent draft to review privately before any eligible in-app message." />}</div></section>}
